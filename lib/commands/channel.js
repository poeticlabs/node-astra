...
module.exports = function ( data, args ) {
	...
	if ( args.length > 0 ) {
		...
		switch ( args[0] ) {
			...
			case 'add':
				...
				async.waterfall( [
					...
					function(channel, callback){
						// Add Channel to DB
						var channel_data = {};
						channel_data.pretty_name = args[1].toLowerCase();

						if ( sails.config.enable_crossover_channel_creation == true ) {
							channel_data.irc = '#' + args[1].toLowerCase();
							channel_data.xmpp = sails.config.xmpp_prefix + args[1].toLowerCase();
						} else {
							if ( data.proto == 'irc' ) {
								channel_data.irc = '#' + args[1].toLowerCase();
			...
			case 'del':
				Channel.findOne( { pretty_name: args[1].toLowerCase() }, function( err, chan ) {
					if ( chan == undefined ) {
			...
			case 'outage':
				...
				if ( sails.config.xmpp.enabled != false ) {
					sails.config.bootstrap.xmpp_client.send(
						new sails.config.bootstrap.xmpp_obj.Element('presence', {
							to: sails.config.xmpp_prefix + args[1] + sails.config.xmpp.chat_domain  + '/' + sails.config.xmpp.local_alias
						}).c('x', { xmlns: 'http://jabber.org/protocol/muc' }).c('history', { maxstanzas: 0, seconds: 1})
					);
				}
							...
							if ( users[i].xmpp != null ) {			
								if ( sails.config.xmpp.enabled != false ) {
									...
									//setTimeout( function() {
										sails.config.bootstrap.xmpp_client.send(
											new sails.config.bootstrap.xmpp_obj.Element('message', {
												to: sails.config.xmpp_prefix + args[1] + sails.config.xmpp.chat_domain
											}).c('x', { xmlns:'http://jabber.org/protocol/muc#user' }).c('invite', {
