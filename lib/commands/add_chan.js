				...
				Channel.create( channel_data ).done(function(err, chan) {
					...
					if ( err ) {
						...
						if ( chan.irc != null ) {
							...
							setTimeout( function() { console.log( 'Setting ChanServ ACCESS for', sails.config.irc.username ) }, 1000 );
							//sails.config.bootstrap.irc_client.say( 'ChanServ', 'ACCESS ' + chan.irc + ' ADD ' + sails.config.irc.username + ' 40');
						}
